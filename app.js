(() => {
  ...
  function renderAssetStrip(){
    ...
    if(visible("tunnels"))for(const tunnel of D.tunnels){if(tunnel.end<left||tunnel.start>right)continue;const x1=sx(tunnel.start),x2=sx(tunnel.end);ctx.strokeStyle="#d5dcd9";ctx.lineWidth=12;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(x1,equipmentY);ctx.lineTo(Math.max(x1+2,x2),equipmentY);ctx.stroke();if(state.zoom>=55){ctx.fillStyle="#7d8983";ctx.font="700 9px sans-serif";ctx.fillText(tunnel.name,(x1+x2)/2+4,equipmentY-8);}for(const [km,edge] of [[tunnel.start,"始点"],[tunnel.end,"終点"]])stripHits.push({kind:"point",x:sx(km),y:equipmentY,km,type:"トンネル"+edge,name:structureName(tunnel)+" "+edge,detail:`${assetKmText(tunnel.start)} — ${assetKmText(tunnel.end)}`,relation:`延長 ${Math.round((tunnel.end-tunnel.start)*1000)}m`});}
    for(const [offset,color] of [[-2,"#b5cfdf"],[2,"#dfb8ba"]]){ctx.strokeStyle=color;ctx.lineWidth=4;ctx.lineCap="butt";ctx.beginPath();ctx.moveTo(0,equipmentY+offset);ctx.lineTo(width,equipmentY+offset);ctx.stroke();}
  ...
  function renderRoute(){
    ...
    if(visible("signals"))for(const s of D.signals){const lineY=routeY(interpolateY(s.km),height),offset=split?(s.direction==="上り"?-19:-33):(s.direction==="上り"?-18:18),yy=lineY+offset,css=`${s.direction==="上り"?"signal-up":"signal-down"}${s.kind==="中継"?" signal-relay":""}`,title=`${s.direction} ${s.kind}信号機 ${s.code||""}`.trim(),labelY=split?yy-6:(s.direction==="上り"?yy-7:yy+14),radius=split?3.2:5;out+=`<g ${attrs("信号機",title,s.km,kmText(s.km),`${s.direction}／${s.kind}`)}><line class="facility-stem" x1="${x(s.km)}" y1="${lineY}" x2="${x(s.km)}" y2="${yy}"/><circle class="${css}" cx="${x(s.km)}" cy="${yy}" r="${radius}"/>`;if(state.zoom>=118||state.layers.signals==="show")out+=`<text class="facility-label" fill="${s.direction==="上り"?"#a72e2a":"#155f99"}" x="${x(s.km)+5}" y="${labelY}">${escapeHtml(s.kind)}</text>`;out+=`</g>`;}
  ...
  function makePlanPrintSvg(start,end,large=false,scaleEnd=end){
    ...
    for(const t of D.tunnels.filter(t=>t.end>=start&&t.start<=end)){const a=Math.max(start,t.start),b=Math.min(end,t.end);out+=`<line x1="${px(a)}" y1="${base}" x2="${px(b)}" y2="${base}" stroke="#d5dcd9" stroke-width="12"/>`+text((a+b)/2,base-22,t.name,'#7d8983',font,'middle');}
    for(const [offset,color] of [[-2,"#b5cfdf"],[2,"#dfb8ba"]])out+=`<line x1="${px(start)}" y1="${base+offset}" x2="${px(end)}" y2="${base+offset}" stroke="${color}" stroke-width="4"/>`;
