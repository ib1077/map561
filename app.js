(() => {
  ...
  function renderAssetStrip(){
    ...
    const width=Math.max(1,assetStrip.clientWidth),height=Math.max(1,assetStrip.clientHeight),ratio=Math.min(2,window.devicePixelRatio||1),ctx=assetStripCanvas.getContext("2d"),laneH=height/2;
    ...
    ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,width,height);ctx.fillStyle="#fcfcfa";ctx.fillRect(0,0,width,height);ctx.strokeStyle="#dedcd6";ctx.lineWidth=1;for(let i=0;i<=2;i+=1){const yy=i*laneH;ctx.beginPath();ctx.moveTo(0,yy+.5);ctx.lineTo(width,yy+.5);ctx.stroke();}
    const sx=km=>PAD+km*state.zoom-routeViewport.scrollLeft,laneY=lane=>laneH*(lane+.5),left=(routeViewport.scrollLeft-PAD)/state.zoom-1,right=(routeViewport.scrollLeft+width-PAD)/state.zoom+1,equipmentY=laneY(1);stripHits=[];
