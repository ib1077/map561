(() => {
  ...
  function renderAssetStrip(){
    ...
    const width=Math.max(1,assetStrip.clientWidth),height=Math.max(1,assetStrip.clientHeight),ratio=Math.min(2,window.devicePixelRatio||1),ctx=assetStripCanvas.getContext("2d"),plotHeight=height-18,equipmentY=plotHeight/2;
    ...
    if(visible("curves"))for(const curve of curveSpans){if(curve.end<left||curve.start>right)continue;const x1=sx(curve.start),x2=sx(curve.end),yy=curve.direction===-1?14:plotHeight-14,inward=curve.direction===-1?7:-7;ctx.strokeStyle="#b17824";ctx.lineWidth=1.8;ctx.lineCap="butt";ctx.beginPath();ctx.moveTo(x1,yy+inward);ctx.lineTo(x1,yy);ctx.lineTo(x2,yy);ctx.lineTo(x2,yy+inward);ctx.stroke();if(curve.radius&&state.zoom>=38){const label=`R${curve.radius}`;ctx.fillStyle="#8b5b16";ctx.font="700 10px sans-serif";ctx.fillText(label,(x1+x2)/2-ctx.measureText(label).width/2,curve.direction===-1?yy-3:yy+11);}stripHits.push({kind:"span",x1,x2,y:yy,km:curve.start,type:"曲線",name:curve.radius?`R${curve.radius}`:"曲線区間",detail:`${assetKmText(curve.start)} — ${assetKmText(curve.end)}`,relation:`延長 ${Math.round((curve.end-curve.start)*1000)}m`});}
    ...
    if(visible("tunnels"))for(const tunnel of D.tunnels){if(tunnel.end<left||tunnel.start>right)continue;const x1=sx(tunnel.start),x2=sx(tunnel.end);ctx.strokeStyle="#d5dcd9";ctx.lineWidth=6;ctx.lineCap="round";ctx.beginPath();ctx.moveTo(x1,equipmentY);ctx.lineTo(Math.max(x1+2,x2),equipmentY);ctx.stroke();if(state.zoom>=55){ctx.fillStyle="#7d8983";ctx.font="700 9px sans-serif";ctx.fillText(tunnel.name,(x1+x2)/2+4,equipmentY-8);}stripHits.push({kind:"span",x1,x2:Math.max(x1+2,x2),y:equipmentY,km:tunnel.start,type:"トンネル",name:structureName(tunnel),detail:`${assetKmText(tunnel.start)} — ${assetKmText(tunnel.end)}`,relation:`延長 ${Math.round((tunnel.end-tunnel.start)*1000)}m`});}
    for(const row of ASSETS){if(row.distanceKm<left)continue;if(row.distanceKm>right)break;if(!stripVisible(row))continue;const xx=sx(row.distanceKm),yy=equipmentY,selected=row.id===state.selectedAssetId,directional=row.category==="信号機"||row.category.includes("地上子"),up=directional&&row.categoryName==="上り",down=directional&&row.categoryName==="下り",markColor=row.category==="無電源地上子"?"#59635f":up?"#a4322f":down?"#26678e":"#59635f",relaySignal=row.category==="信号機"&&row.attribute==="中継";ctx.lineWidth=relaySignal?1.05:1.1;ctx.strokeStyle=markColor;ctx.fillStyle=markColor;ctx.beginPath();if(row.category==="信号機"){ctx.arc(xx,yy,selected?4.5:3.4,0,Math.PI*2);ctx.fillStyle=relaySignal?"#fcfcfa":markColor;ctx.fill();ctx.stroke();}else if(row.category==="無電源地上子"){ctx.rect(xx-(selected?4:3),yy-1.5,selected?8:6,3);ctx.fill();}else{const size=selected?3.2:2.2;ctx.rect(xx-size,yy-size,size*2,size*2);ctx.fill();}if(selected){ctx.beginPath();ctx.arc(xx,yy,6,0,Math.PI*2);ctx.strokeStyle="#d18d20";ctx.lineWidth=1.3;ctx.stroke();}const label=stripLabel(row);if(label&&stripLabelVisible(row)){ctx.fillStyle=markColor;ctx.font="700 9px sans-serif";ctx.fillText(label,xx+4,up?yy+14:yy-7);}stripHits.push({kind:"point",x:xx,y:yy,row});}
  ...
  function renderRoute(){
    ...
    for(let km=0;km<=TOTAL+.0001;km+=step){const n=Math.round(km*10)/10,whole=Math.abs(n-Math.round(n))<.001,half=!whole&&Math.abs(n*2-Math.round(n*2))<.001;out+=`<line class="rail-grid${whole?" major":""}" x1="${x(n)}" y1="22" x2="${x(n)}" y2="${height-28}"/>`;if(whole)out+=`<text class="km-label" x="${x(n)+3}" y="18">${Math.round(n)}k</text>`;else if(half&&state.zoom>=55)out+=`<text class="km-label" x="${x(n)+3}" y="18">1/2</text>`;}
  ...
  function selectTrain(id){if(!SPEED_PROFILES[id])return;state.selectedTrain=id;$("#trainSelect").value=id;updateTimeCalculation(state.timeStart,state.timeEnd);updatePrintCalculationToggle();renderRoute();renderSpeed();save();toast(`${trainLabel()}の速度を表示します`);}
  ...
  function setNavigatorOpen(open){document.body.classList.toggle("navigator-open",Boolean(open));$("#navigatorBtn").classList.toggle("active",Boolean(open));$("#navigatorBtn").setAttribute("aria-expanded",String(Boolean(open)));if(open)requestAnimationFrame(()=>positionSlider.focus({preventScroll:true}));}
  ...
  function gestures(v,strip=false){
    ...
    surface.addEventListener("pointermove",e=>{if(!pointers.has(e.pointerId))return;const p=point(e);pointers.set(e.pointerId,p);if(pointers.size>=2){const [a,b]=[...pointers.values()],origin=strip?{left:0,top:0}:gestureOrigin(v);state.zoom=Math.max(fitZoom(),Math.min(600,gesture.zoom*Math.hypot(a.x-b.x,a.y-b.y)/gesture.distance));renderRoute();renderSpeed();v.scrollLeft=Math.max(0,PAD+gesture.km*state.zoom-(a.x+b.x)/2+origin.left);if(!strip)v.scrollTop=Math.max(0,gesture.contentY-(a.y+b.y)/2+origin.top);}else{if(!gesture||gesture.kind!=="pan")beginPan(p);const dx=p.x-gesture.startX,dy=p.y-gesture.startY;if(Math.hypot(dx,dy)>5){gesture.moved=true;suppressClick=true;}routeMotion.move(p.x,p.y);v.scrollLeft=Math.max(0,gesture.scrollLeft-dx);if(!strip)v.scrollTop=Math.max(0,gesture.scrollTop-dy);}updateNavigator();queueStripRender();});
    const finish=e=>{if(!pointers.has(e.pointerId))return;const g=gesture;pointers.delete(e.pointerId);if(pointers.size){beginPan([...pointers.values()][0]);return;}gesture=null;const normal=e.type==="pointerup";if(normal&&!pinched&&g?.moved)routeMotion.end();else routeMotion.reset();save();if(strip&&normal&&!suppressClick&&!pinched&&g&&!g.moved){const p=point(e);let best=null,dist=Infinity;for(const h of stripHits){const d=h.kind==="span"&&p.x>=Math.min(h.x1,h.x2)-5&&p.x<=Math.max(h.x1,h.x2)+5?Math.abs(p.y-h.y):h.kind==="point"?Math.hypot(p.x-h.x,p.y-h.y):Infinity;if(d<dist){best=h;dist=d;}}if(best&&dist<=14)showStripHit(best);}};
  ...
  $("#fitBtn").addEventListener("click",fitAll);$("#zoomInBtn").addEventListener("click",()=>setZoom(state.zoom*1.55));$("#zoomOutBtn").addEventListener("click",()=>setZoom(state.zoom/1.55));$("#navigatorBtn").addEventListener("click",()=>setNavigatorOpen(!document.body.classList.contains("navigator-open")));$("#infoClose").addEventListener("click",closeInfo);
  positionSlider.addEventListener("input",()=>scrollCenter(Number(positionSlider.value),true));positionSlider.addEventListener("change",()=>{save();setNavigatorOpen(false);});
