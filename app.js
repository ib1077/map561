(() => {
  ...
  function gestures(v){
    ...
    const beginPinch=()=>{const [a,b]=values(),origin=gestureOrigin(v),midX=(a.x+b.x)/2-origin.left,midY=(a.y+b.y)/2-origin.top,distance=Math.hypot(a.x-b.x,a.y-b.y);gesture={kind:"pinch",distance:Math.max(1,distance),zoom:state.zoom,km:(v.scrollLeft+midX-PAD)/state.zoom,contentY:v.scrollTop+midY};};
    v.addEventListener("pointerdown",e=>{v.closest('.workspace').classList.add('used');v.setPointerCapture?.(e.pointerId);pointers.set(e.pointerId,logicalPointer(e));if(pointers.size===1)beginPan(values()[0]);else if(pointers.size===2)beginPinch();});
  ...
  assetStrip.addEventListener("pointerdown",event=>{if(!lowerPanelVisible()||event.target.closest("button"))return;const point=logicalPointer(event,assetStrip);assetStrip.setPointerCapture?.(event.pointerId);stripPointers.set(event.pointerId,point);if(stripPointers.size===1)stripGesture={kind:"pan",id:event.pointerId,startX:point.x,startY:point.y,startScroll:routeViewport.scrollLeft,moved:false};else if(stripPointers.size===2){const [a,b]=[...stripPointers.values()],mid=(a.x+b.x)/2;stripGesture={kind:"pinch",distance:Math.max(1,Math.hypot(a.x-b.x,a.y-b.y)),zoom:state.zoom,km:(routeViewport.scrollLeft+mid-PAD)/state.zoom,moved:true};}});
  assetStrip.addEventListener("pointermove",event=>{if(!stripPointers.has(event.pointerId)||!stripGesture)return;const point=logicalPointer(event,assetStrip);stripPointers.set(event.pointerId,point);if(stripPointers.size>=2){const [a,b]=[...stripPointers.values()],distance=Math.hypot(a.x-b.x,a.y-b.y),mid=(a.x+b.x)/2;state.zoom=Math.max(fitZoom(),Math.min(600,stripGesture.zoom*distance/stripGesture.distance));renderRoute();renderSpeed();routeViewport.scrollLeft=Math.max(0,PAD+stripGesture.km*state.zoom-mid);updateNavigator();queueStripRender();}else if(stripGesture.kind==="pan"){const dx=point.x-stripGesture.startX;if(Math.abs(dx)>5)stripGesture.moved=true;routeViewport.scrollLeft=Math.max(0,stripGesture.startScroll-dx);}});
  const finishStripGesture=event=>{if(!stripPointers.has(event.pointerId))return;const gesture=stripGesture;stripPointers.delete(event.pointerId);if(stripPointers.size){const point=[...stripPointers.entries()][0];stripGesture={kind:"pan",id:point[0],startX:point[1].x,startY:point[1].y,startScroll:routeViewport.scrollLeft,moved:true};return;}stripGesture=null;save();if(!gesture||gesture.moved)return;const point=logicalPointer(event,assetStrip),px=point.x,py=point.y;let best=null,bestDistance=Infinity;for(const hit of stripHits){const distance=hit.kind==="span"&&px>=Math.min(hit.x1,hit.x2)-5&&px<=Math.max(hit.x1,hit.x2)+5?Math.abs(py-hit.y):hit.kind==="point"?Math.hypot(px-hit.x,py-hit.y):Infinity;if(distance<bestDistance){best=hit;bestDistance=distance;}}if(best&&bestDistance<=14)showStripHit(best);};
